...
fn main() {
    ...
    println!("cell_id;x;y");
    // Lane A: cells 0-4 (y=1.0)
    for i in 0..5 {
        let mut cell = Cell::new(i as i64);
        if i < 4 {
            cell = cell.with_forward_node((i + 1) as i64);
        } else {
            // Last cell in lane A connects to start of merged lane
            cell = cell.with_right_node(10);
        }
        cell = cell
    ...
    // Lane B: cells 5-9 (y=2.0)
    for i in 0..5 {
        let cell_id = (i + 5) as i64;
        let mut cell = Cell::new(cell_id);
        if i < 4 {
            cell = cell.with_forward_node(cell_id + 1);
        } else {
            // Last cell in lane B connects to start of merged lane
            cell = cell.with_left_node(10);
        }
        cell = cell
            .with_point(new_point(i as f64, 2.0, None))
            .with_speed_limit(4);
        let c = cell.build();
        let pt = c.get_point();
        println!("{};{:.5};{:.5}", c.get_id(), pt.x(), pt.y());
    ...
    // Merged lane: cells 10-14 (y=1.5)
    for i in 0..5 {
        let cell_id = (i + 10) as i64;
        let mut cell = Cell::new(cell_id);
        if i < 4 {
            cell = cell.with_forward_node(cell_id + 1);
        }
        cell = cell
            .with_point(new_point((i + 5) as f64, 1.5, None))
            .with_speed_limit(4);
        let c = cell.build();
        let pt = c.get_point();
        println!("{};{:.5};{:.5}", c.get_id(), pt.x(), pt.y());
    ...
    // 2. Create two vehicles, one per lane, each with its own destination
    let vehicle0 = Vehicle::new(0)
        .with_speed(1)
        .with_speed_limit(1)
        .with_cell(0)
        .with_destination(14)
    ...
    let vehicle1 = Vehicle::new(1)
        ...
        .with_destination(14)
        .build();
    let vehicles: Vec<VehicleRef> = vec![
        Rc::new(RefCell::new(vehicle0)),
        Rc::new(RefCell::new(vehicle1)),
    ];
