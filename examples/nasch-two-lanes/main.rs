...
fn main() {
    ...
    for i in 0..lane_length {
        ...
        // Allow right maneuver to lane 1
        cell = cell.with_right_node((i + lane_length) as i64);
        // Always set point and speed limit
        cell = cell
    ...
    // Lane 1 (y = 2.0)
    for i in 0..lane_length {
        let cell_id = (i + lane_length) as i64;
        let mut cell = Cell::new(cell_id);
        if i < lane_length - 1 {
            cell = cell.with_forward_node((cell_id + 1) as i64);
        }
        // Allow left maneuver to lane 0
        cell = cell.with_left_node((i) as i64);
        // Always set point and speed limit
        cell = cell
    ...
    let vehicle0 = Vehicle::new(0)
        ...
        .with_speed_limit(1)
        .with_cell(0)
        .with_destination(last_cell_id_lane0)
        .build();
    let vehicle1 = Vehicle::new(1)
        .with_speed(1)
        .with_speed_limit(1)
        .with_cell(lane_length as i64)
        .with_destination(last_cell_id_lane1)
        .build();
