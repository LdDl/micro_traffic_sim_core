...
fn main() {
    ...
    println!("cell_id;x;y");
    // Lane A: cells 0-4 (45 deg, up and right)
    for i in 0..5 {
        let mut cell = Cell::new(i as i64);
        if i < 4 {
            cell = cell.with_forward_node((i + 1) as i64);
        } else {
            cell = cell.with_forward_node(10);
        }
        // 45 deg: (i, i+1)
        cell = cell
            .with_point(new_point(i as f64, (i + 1) as f64, None))
            .with_speed_limit(4);
        let c = cell.build();
        let pt = c.get_point();
    ...
    // Lane B: cells 5-9 (-45 deg, down and right)
    for i in 0..5 {
        ...
        // -45 deg: (i, 10 - i)
        cell = cell
            .with_point(new_point(i as f64, (10 - i) as f64, None))
            .with_speed_limit(4);
        let c = cell.build();
        let pt = c.get_point();
        println!("{};{:.5};{:.5}", c.get_id(), pt.x(), pt.y());
    ...
    // Merged lane: cells 10-14 (horizontal, y=5.5)
    for i in 0..5 {
        ...
        // Merge lane: (i+5, 5.5)
        cell = cell
            .with_point(new_point((i + 5) as f64, 5.5, None))
            .with_speed_limit(4);
        let c = cell.build();
        let pt = c.get_point();
        println!("{};{:.5};{:.5}", c.get_id(), pt.x(), pt.y());
