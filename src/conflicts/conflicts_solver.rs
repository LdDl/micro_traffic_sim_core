...
pub fn solve_conflicts(
    ...
    let mut trajectories_conflicts: Vec<CellConflict<'_>> = Vec::new();
    let mut conflict_zones_conflicts: Vec<CellConflict<'_>> = Vec::new();
    for mut conflict in conflicts_data {
        ...
        match conflict.conflict_type {
            ...
            ConflictType::CrossLaneChange | ConflictType::TailCrossLaneChange => {
                ...
                if let Some(priority_participant) = conflict.participants.get_mut(conflict.priority_participant_index) {
    ...
    for mut conflict in trajectories_conflicts {
        if verbose {
            let vehicles: Vec<_> = conflict.participants.iter().map(|p| p.id).collect();
        ...
        if need_to_solve && left_maneuver_index.is_some() && right_maneuver_index.is_some() {
            ...
            if let Some(right_index) = right_maneuver_index {
                conflict.participants[right_index].block_with_speed(0);
            }
            ...
            if let Some(left_index) = left_maneuver_index {
                conflict.participants[left_index].intention.intention_speed = 1;
            }
        }
    }
    ...
    for mut conflict in conflict_zones_conflicts {
        ...
        for (i, participant) in conflict.participants.iter_mut().enumerate() {
            if i != priority_index {
                participant.block_with_speed(1);
            }
        }
    }
    Ok(())
...
mod tests {
    use super::*;
    use crate::agents::{Vehicle, VehicleIntention, BehaviourType};
    use crate::grid::lane_change_type::LaneChangeType;
    use crate::grid::cell::CellID;
    ...
    fn test_solve_conflicts_insufficient_participants() {
        let mut vehicle = create_test_vehicle(1,2, LaneChangeType::NoChange, 5);
        
        let conflicts = vec![CellConflict {
            cell_id: 5,
            participants: vec![&mut vehicle], // Only one participant
            priority_participant_index: 0,
    ...
    fn test_solve_conflicts_invalid_priority_index() {
        let mut vehicle1 = create_test_vehicle(1, 2, LaneChangeType::NoChange, 5);
        let mut vehicle2 = create_test_vehicle(2, 2, LaneChangeType::NoChange, 5);
    ...
    fn test_solve_conflicts_common_conflict() {
        let mut priority_vehicle = create_test_vehicle(1, 3, LaneChangeType::ChangeLeft, 5);
        let mut blocked_vehicle = create_test_vehicle(2, 2, LaneChangeType::NoChange, 5);
        
        let conflicts = vec![CellConflict {
            cell_id: 5,
            participants: vec![&mut priority_vehicle, &mut blocked_vehicle],
        ...
        assert_eq!(priority_vehicle.intention.intention_cell_id, 5, "Priority vehicle should have target cell");
        assert_eq!(priority_vehicle.intention.intention_speed, 1, "Priority vehicle should have speed 1");
        assert_eq!(blocked_vehicle.intention.intention_speed, 0, "Blocked vehicle should have speed 0");
    }

    #[test]
    fn test_solve_conflicts_trajectories_conflict() {
        let mut left_vehicle = create_test_vehicle(1, 2, LaneChangeType::ChangeLeft, 5);
        ...
        assert_eq!(right_vehicle.intention.intention_speed, 0, "Right vehicle should be blocked");
        assert_eq!(left_vehicle.intention.intention_speed, 1, "Left vehicle should continue");
    }

    #[test]
    fn test_solve_conflicts_conflict_zone() {
        let mut priority_vehicle = create_test_vehicle(1,2, LaneChangeType::NoChange, 5);
        let mut blocked_vehicle = create_test_vehicle(2, 2, LaneChangeType::NoChange, 6);
        ...
        assert_eq!(blocked_vehicle.intention.intention_speed, 1, "Blocked vehicle in conflict zone should have speed 1");
    }

    #[test]
    fn test_solve_conflicts_multiple_conflicts() {
        let mut vehicle1 = create_test_vehicle(1, 2, LaneChangeType::ChangeLeft, 5);
        let mut vehicle2 = create_test_vehicle(2, 2, LaneChangeType::NoChange, 5);
        let mut vehicle3 = create_test_vehicle(3, 1, LaneChangeType::ChangeRight, 7);
        let mut vehicle4 = create_test_vehicle(4, 1, LaneChangeType::NoChange, 7);
        ...
        let conflicts = vec![
            ...
            CellConflict {
                cell_id: 7,
                participants: vec![&mut vehicle3, &mut vehicle4],
        ...
        assert_eq!(vehicle1.intention.intention_cell_id, 5, "Vehicle 1 should get target cell");
        assert_eq!(vehicle1.intention.intention_speed, 1, "Vehicle 1 should have speed 1");
        assert_eq!(vehicle2.intention.intention_speed, 0, "Vehicle 2 should be blocked");
        
        // Check second conflict resolution
        assert_eq!(vehicle3.intention.intention_speed, 0, "Vehicle 3 should be blocked");
        assert_eq!(vehicle4.intention.intention_speed, 1, "Vehicle 4 should keep its original speed (has priority, NoChange)");
    }
}